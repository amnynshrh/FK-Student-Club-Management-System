...
require_login(['student','committee','admin']); update_event_status($conn); $mode=$_GET['mode']??'student'; $eventId=$_GET['id']??''; if($eventId===''){die('Event ID is required.');}
$st=mysqli_prepare($conn,"SELECT e.*, c.club_name, COUNT(er.registration_id) registered_count FROM event e INNER JOIN club c ON c.club_id=e.club_id LEFT JOIN eventregistration er ON er.event_id=e.event_id AND er.registration_status='registered' WHERE e.event_id=? GROUP BY e.event_id"); mysqli_stmt_bind_param($st,'i',$eventId); mysqli_stmt_execute($st); $event=mysqli_fetch_assoc(mysqli_stmt_get_result($st)); if(!$event){die('Event not found.');}
$p=mysqli_prepare($conn,"SELECT s.name,s.matric_number,s.course,u.email,COALESCE(a.attendance_status,'not marked') attendance_status FROM eventregistration er INNER JOIN student s ON s.matric_number=er.matric_number INNER JOIN user u ON u.user_id=s.user_id LEFT JOIN attendance a ON a.registration_id=er.registration_id WHERE er.event_id=? AND er.registration_status='registered' ORDER BY er.registration_date ASC"); mysqli_stmt_bind_param($p,'i',$eventId); mysqli_stmt_execute($p); $participants=mysqli_stmt_get_result($p);
$w=mysqli_prepare($conn,"SELECT s.name,s.matric_number,ew.waiting_status FROM eventwaitinglist ew INNER JOIN student s ON s.matric_number=ew.matric_number WHERE ew.event_id=? AND ew.waiting_status IN ('waiting','notified') ORDER BY ew.joined_at ASC"); mysqli_stmt_bind_param($w,'i',$eventId); mysqli_stmt_execute($w); $waiting=mysqli_stmt_get_result($w);
?>
<!DOCTYPE html>
        ...
        <div class="view-event-page">
            ...
            <section class="view-event-card">
                ...
                <div class="view-event-details-grid">
                    ...
                    <div class="view-detail-item">
                        ...
                        <div>
                            ...
                            <?php echo badge($event['event_status']); ?>
                        </div>
                    </div>
                ...
                <div class="view-event-actions">
                    <a href="committee_manage_events.php" class="view-event-back-btn">
            ...
            <?php if($mode === 'committee'): ?><section class="view-event-card participant-list-card">
                ...
                <div class="table-responsive">
                    <table class="event-table-custom">
                        ...
                        <tbody id="participantTableBody"><?php while($row=mysqli_fetch_assoc($participants)): ?><tr><td><?php echo e($row['name']); ?></td><td><?php echo e($row['matric_number']); ?></td><td><?php echo e($row['course']); ?></td><td><?php echo e($row['email']); ?></td><td><?php echo e($row['attendance_status']); ?></td></tr><?php endwhile; ?></tbody>
                    </table>
                </div>
            </section>
            ...
            <section class="view-event-card participant-list-card">
                ...
                <div class="table-responsive">
                    <table class="event-table-custom">
                        ...
                        <tbody id="waitingTableBody"><?php while($row=mysqli_fetch_assoc($waiting)): ?><tr><td><?php echo e($row['name']); ?></td><td><?php echo e($row['matric_number']); ?></td><td><?php echo e($row['waiting_status']); ?></td></tr><?php endwhile; ?></tbody>
                    </table>
                </div>
            </section>
        </div>
