...
// Make sure committee pages always have the committee_id, even if the login
// session was created before this value was added.
$committee_id = $_SESSION['SESS_COMMITTEE_ID'] ?? null;
if (empty($committee_id)) {
    ...
    if ($stmt_committee) {
        $stmt_committee->bind_param("i", $user_id);
        ...
        if (!empty($committee_data['committee_id'])) {
            $committee_id = (int) $committee_data['committee_id'];
            $_SESSION['SESS_COMMITTEE_ID'] = $committee_id;
        }
    }
}

$committee_id = (int) ($committee_id ?? 0);
