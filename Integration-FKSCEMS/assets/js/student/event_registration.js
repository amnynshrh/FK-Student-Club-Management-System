...
function displayPage() {
  ...
  eventsToShow.forEach(function (event) {
    ...
    row.innerHTML = `
      ...
      <td>
        <a href="view_event.php?id=${event.id}" class="btn-view-event" title="View Details">
          <i class="bi bi-eye"></i>
        </a>
...
function renderActionButton(event) {
  ...
  if (event.isWaiting) {
    return `
      ...
      <a href="student/event_register_form.html?id=${event.id}" class="btn-register-event">Register</a>
    `;
  }

  return `<a href="student/event_register_form.html?id=${event.id}" class="btn-register-event">Register</a>`;
}
      ...
      fetch("api/student/events/cancel_registration.php", {
        ...
        .then(function (data) {
          if (data.success) {
            ...
            window.location.href = "event_registration.php";
          } else {
            alert(data.message);
          }
        })
...
function formatDisplayTime(timeValue) {
  ...
  if (timePart.includes(" ")) {
    timePart = timePart.split(" ").pop();
  } else if (timePart.includes("T")) {
    timePart = timePart.split("T")[1].split(/[Z+-]/)[0];
  }

  const timeParts = timePart.split(":");
  let hour = parseInt(timeParts[0], 10);
  const minute = (timeParts[1] || "00").substring(0, 2);
